...
class ContentPane extends Component {
  ...
  render() {
    ...
    return (
      ...
      {hasWork && (<Grid>
        <Grid.Column width="8">
            <ToDoPane key={`t${toDoWorks.length}`}
             toDoWorks={toDoWorks} workDateId={workDateId} />
        </Grid.Column>
        <Grid.Column width="8">
            <DonePane key={`t${doneWorks.length}`}
             doneWorks={doneWorks} workDateId={workDateId}/>
        </Grid.Column>
      </Grid>)}
      {!hasWork &&
      (
        <EmptyContentMessage workDate={this.props.workDate}/>
      )}
      </Fragment>
    );
  }
